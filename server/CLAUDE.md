...
server/
  ...
  knowledge_base.py       # disease KB, RAG retrieval, synthetic patient generation
  chat.py                 # legacy — superseded by session.py, ignore
  inference.py            # legacy — superseded by model.py + session.py, ignore
  diseases.json           # NOT YET PRESENT — will be built offline via Human Delta
...
5. Teacher hits Start Session — frontend sends disease, symptoms, and vitals ranges to `POST /session/new`
6. Backend generates a synthetic patient (random name, age, gender, backstory, vitals drawn from the teacher-set ranges) and stores everything in the global session
...
### app.py
Owns the FastAPI app instance, route registration, CORS middleware, and the lifespan handler that calls `model.load()` at startup. Route handlers are thin — they delegate to `session.py` and `report.py`. No business logic lives here.

### model.py
Owns model loading and raw inference only. Loads the base Qwen 2.5-1.5B model and the LoRA adapter from `qwen-patient-adapter/` once at startup. Exposes `load()` (called once) and `generate_patient_response(history, patient, max_new_tokens)` which returns a plain string. Includes a post-generation identity guard to fix name/age hallucinations. No prompt logic lives here.
...
### report.py
Owns OSCE evaluation. Builds the evaluation prompt, calls Groq, parses the JSON response, recomputes the total score, and returns the full report dict. The `_OSCE_PROMPT` template is a module-level constant. No route logic lives here.

### knowledge_base.py
Owns all disease data and patient generation. Currently loads from the `QuyenAnhDE/Diseases_Symptoms` HuggingFace dataset at startup with a small static fallback.

**The knowledge base loading will be replaced.** A `diseases.json` file built offline via Human Delta (a web crawler and vector search API) will become the source of truth. Human Delta crawls authoritative medical sources (NHS, CDC), indexes the content, and allows semantic search over it. The build process queries each disease, extracts structured symptom and vitals data from the search results using an LLM, and writes it to `diseases.json`. This build process lives in a separate script and is not part of the running app. The running app will only read `diseases.json` — it will never call Human Delta at runtime. **Do not change the KB loading logic until diseases.json is ready.**

Key public functions that must always exist:
- `list_diseases()` → sorted list of disease name strings
- `disease_info(name)` → dict with `disease`, `symptoms`, `treatments`, `vitals_ranges`
- `disease_vitals_ranges(disease)` → dict of `{key: {min, max, unit}}`
- `rag_retrieve(disease, question)` → list of relevant symptom strings
- `synthea_patient()` → fully random synthetic patient dict
- `synthea_patient_from_spec(disease, symptoms, vitals_ranges)` → patient from teacher spec
...
| `GET` | `/diseases/{name}` | Symptoms, treatments, and vitals ranges for a disease |
| `POST` | `/session/new` | Create patient from teacher spec; reset global session |
...
- Do not change the knowledge base loading until `diseases.json` is ready
- Use `uv`, not `pip`
